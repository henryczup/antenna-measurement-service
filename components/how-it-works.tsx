      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="grid md:grid-cols-3 gap-10 max-w-6xl mx-auto">
          {problems.map((problem, index) => (
            <div
              key={index}
              className="relative h-full rounded-3xl border border-white/10 bg-white/5 p-6 transition duration-300 hover:border-primary/60 hover:bg-white/10"
            >
              <div className="flex flex-col items-center text-center">
                ...
                <h3 className="text-xl font-sentient text-white mb-3">{problem.title}</h3>
                <p className="text-base text-white/70 leading-relaxed font-light">{problem.description}</p>
              </div>
            </div>
          ))}
        </div>
